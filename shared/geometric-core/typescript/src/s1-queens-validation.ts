...
import type { S0Output } from './s0-akron-intake'

export interface S1Input extends S0Output {
  // Inherits from S0 output
}
 ...
 */
function calculateCoherence(input: any): number {
 ...
 */
export async function s1_queens_validation(input: S1Input): Promise<S1Output> {
  ...
  if (!passed) {
    throw new Error(
      `S1 validation failed: coherence ${coherence.toFixed(2)} < threshold ${COHERENCE_THRESHOLD}`
    )
  }
 ...
 */
export function setCoherenceCalculator(
  calculator: (input: any) => number
): void {
