...
function checkAnchor(artifact) {
    const missing = [];
    if (!artifact.path || artifact.path.trim() === '')
        missing.push('path');
    if (!artifact.lineage.canonicalRef || artifact.lineage.canonicalRef.trim() === '')
        missing.push('canonicalRef');
    if (!artifact.timestamp || artifact.timestamp.trim() === '')
        missing.push('timestamp');
    if (!artifact.actor || artifact.actor.trim() === '')
        missing.push('actor');
    if (missing.length > 0) {
        return {
            passed: false,
...
function checkScope(artifact) {
    ...
    if (artifact.fieldRegistry && artifact.content) {
        ...
        const phantomEntries = Object.keys(artifact.content).filter(k => !registryKeys.has(k));
...
function checkLineage(lineage) {
    const missing = [];
    if (!lineage.canonicalRef || lineage.canonicalRef.trim() === '')
        missing.push('canonicalRef');
    if (!lineage.version || lineage.version.trim() === '')
        missing.push('version');
    if (!lineage.status || lineage.status.trim() === '')
        missing.push('status');
    if (missing.length > 0) {
        return {
            passed: false,
...
function checkConstraints(artifact) {
    const violations = [];
...
// ---------------------------------------------------------------------------
function buildPatchPlan(checks) {
    const steps = ['[ROLLBACK-SAFE PATCH PLAN] Do NOT merge. Resolve the following before re-evaluating:'];
    let stepNum = 1;
 ...
 * ```
 */
export async function evaluateArtifact(artifact) {
...
//# sourceMappingURL=artifact-evaluator.js.map